...
const ServicesSection: React.FC = () => {
  ...
  const openDemoModal = (url: string) => {
  ...
  return (
    <section id="services" className="py-20 md:py-28 bg-gray-800 text-gray-100">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {SERVICES.map((service, index) => (
            <div
              key={index}
              className="bg-gray-900 p-8 rounded-lg shadow-xl hover:shadow-2xl transition-all duration-300 ease-in-out transform hover:-translate-y-2 hover:border-indigo-500 border border-transparent animate-zoomIn flex flex-col items-center text-center"
            ...
            >
              ...
              {service.demoLink && (
                <Button variant="outline" onClick={() => openDemoModal(service.demoLink)} className="mt-auto">
                  Watch Demo
                </Button>
              )}
            </div>
      ...
      {isModalOpen && (
        <Modal onClose={closeDemoModal} title="Service Demo">
